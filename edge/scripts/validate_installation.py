...
def run_component_validations():
    ...
    try:
        result = subprocess.run([sys.executable, 'edge/scripts/validate_easyocr.py'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
    ...
    try:
        result = subprocess.run([sys.executable, 'edge/scripts/validate_database.py'], 
                              capture_output=True, text=True)
        if result.returncode == 0:
