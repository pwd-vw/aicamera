...
# Type definitions for enhanced API
...
class CameraHandler:
    ...
    def __init__(self):
        ...
        self.logger.info("CameraHandler Singleton initialized with enhancement engine")
    ...
    def update_configuration(self, config: Dict[str, Any]) -> bool:
        """
        Update camera configuration using safe reconfiguration workflow.
        ...
        Returns:
            bool: True if successful, False otherwise
        """
        return self.reconfigure_camera_safely(config)
    ...
    def close_camera(self) -> bool:
        ...
        try:
            with self._lock:
                self.logger.info("Closing camera with full resource cleanup...")
                ...
                self.logger.info("Camera closed and all resources released")
                return True
    ...
    def _release_all_resources(self):
        """Internal method to release all camera resources completely."""
        try:
            self.logger.debug("Releasing all camera resources...")
            
            # Stop everything
            self.close_camera()
            
            self.logger.debug("All camera resources released")
    ...
    def cleanup_singleton(cls):
        """
        Cleanup singleton instance and release all resources.
        ...
        with cls._lock:
            if cls._instance:
                try:
                    ...
                    cls._instance = None
                    logger.info("Camera singleton cleaned up")
                except Exception as e:
                    logger.error(f"Error cleaning up singleton: {e}")
...
# =============================================================================
# OPTIMIZATION ENHANCEMENTS v2.0
# =============================================================================
"""
Camera Handler Optimizations Applied:

1. UNIFIED CAPTURE METHOD:
   - capture_frame() now supports multiple sources ("buffer", "direct")
   - Supports stream types ("main", "lores", "both") 
   - Quality modes ("normal", "low_light", "bright", "high_quality")
   - Backward compatible with existing code

2. CAMERA ENHANCEMENTS:
   - CameraEnhancementEngine for autofocus and environmental adaptation
   - Automatic lighting condition detection and optimization
   - Intelligent autofocus based on focus quality metrics
   - Noise reduction and exposure optimization

3. IMPROVED SINGLETON PATTERN:
   - Thread-safe double-checked locking
   - Proper resource cleanup on exit
   - Singleton cleanup method for testing

4. ENHANCED STATUS & METADATA:
   - Comprehensive camera status with diagnostics
   - Enhanced metadata with quality assessments
   - Buffer status monitoring
   - Enhancement status tracking

5. OPTIMIZED RESOURCE MANAGEMENT:
   - Enhanced close_camera() with complete resource cleanup
   - Proper stop/release workflow
   - Thread-safe buffer management
   - Memory leak prevention

6. RECONFIGURATION BEST PRACTICES:
   - reconfigure_camera_safely() follows best practices:
...
Usage Examples:
   ...
   # Enhanced usage with unified method
   frame_data = camera.capture_frame(
       source="direct",
       stream_type="main", 
   ...
   # Includes: diagnostics, enhancement_status, buffer_status
